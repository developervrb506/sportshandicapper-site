...
.art-hero-section {
    width:100%;
    background:linear-gradient(180deg,#0c1226 0%,var(--bg) 100%);
...
/* ── Body wrapper ── */
.art-outer { max-width:1160px; margin:0 auto; }
.art-excerpt-bar {
...
.art-body h3 {
    font-weight:700;
    margin:24px 0 10px;text-transform:uppercase;letter-spacing:.04em;font-size:.85rem;
    color:#60a5fa;
}
...
.art-body ul li::before { content:'›';position:absolute;left:0;color:var(--accent);font-size:1.1em;font-weight:700; }
...
.sb-panel-hdr-title { font-size:11px;color:var(--accent);text-transform:uppercase;letter-spacing:.7px;font-weight:700; }
.sb-label { font-size:10px;color:var(--accent);text-transform:uppercase;letter-spacing:.6px;font-weight:700;margin-bottom:7px; }

/* ── Related ── */
.related-grid { display:grid;grid-template-columns:repeat(3,1fr);gap:14px;margin-top:16px; }
.related-card {
    background:var(--inner);border:1px solid var(--bdr);
...
.pick-card {
    ...
    background:var(--card);border:1px solid rgba(30,144,255,.2);
    border-radius:14px;
}
...
@media(max-width:600px){
    .art-hero-title { font-size:1.3rem; }
    .art-hero-section { margin:-16px -16px 20px; }
    .art-hero-inner { padding:24px 16px 20px; }
    .art-hero-meta { gap:0; }
...
$sportMeta = [
    'nfl'   => ['bg'=>'rgba(29,78,216,.2)',   'color'=>'#93c5fd'],
    'mlb'   => ['bg'=>'rgba(22,163,74,.2)',    'color'=>'#4ade80'],
    'nba'   => ['bg'=>'rgba(220,38,38,.2)',    'color'=>'#f87171'],
    'nhl'   => ['bg'=>'rgba(30,144,255,.2)',   'color'=>'#60a5fa'],
    'ncaaf' => ['bg'=>'rgba(245,158,11,.2)',   'color'=>'#fcd34d'],
    'ncaab' => ['bg'=>'rgba(168,85,247,.2)',   'color'=>'#c084fc'],
];
$sm = $sportMeta[$sp] ?? ['bg'=>'rgba(30,144,255,.15)','color'=>'#60a5fa'];
...
<div class="art-hero-section">
    <div class="art-hero-inner">
        ...
        <div class="art-breadcrumb">
            <a href="/subscriber/articles">Articles</a>
    ...
    <div class="art-columns">
        ...
        <main class="art-main">
            ...
            <div class="art-card" style="padding:0;overflow:hidden;">
                ...
                @endif
                <div class="art-body content" style="padding:32px 36px;">
                    {!! $article->content !!}
                </div>
            </div>
            ...
            <div style="margin-top:40px;">
                <div style="font-size:10px;color:var(--accent);text-transform:uppercase;letter-spacing:.7px;font-weight:700;margin-bottom:16px;">More in {{ $article->sport }}</div>
                <div class="related-grid">
                    @foreach($related as $r)
                    @php
                        $rs = strtolower($r->sport ?? '');
                        $rsm = $sportMeta[$rs] ?? ['bg'=>'rgba(30,144,255,.1)','color'=>'#60a5fa'];
                    @endphp
                    <a href="/subscriber/articles/{{ $r->slug }}" class="related-card">
                        <div style="position:relative;aspect-ratio:16/9;background:#0a0f1e;overflow:hidden;">
                            @if($r->featured_image)
                ...
                @foreach($article->supplements as $sup)
                    ...
                    <div class="sb-panel">
                        ...
                        <div style="padding:13px 15px;">
                            ...
                            <div style="display:flex;gap:9px;margin-bottom:8px;font-size:13px;color:#94A3B8;line-height:1.55;">
                                <span style="color:var(--accent);flex-shrink:0;font-size:14px;margin-top:-1px;">›</span><span>{{ $ins }}</span>
                            </div>
                            @endforeach
                        </div>
                    </div>
                    ...
                    <div class="sb-panel">
                        ...
                        <div style="padding:13px 15px;display:flex;flex-direction:column;gap:8px;">
                            ...
                            <div style="background:rgba(30,144,255,.05);border-left:3px solid var(--accent);padding:9px 12px;border-radius:0 7px 7px 0;">
                                <div style="font-size:9px;color:var(--accent);font-weight:700;margin-bottom:4px;letter-spacing:.4px;text-transform:uppercase;">Public Bettors</div>
                    ...
                    <div class="sb-panel">
                        ...
                        <div style="padding:12px 14px;">
                            ...
                            @php $cid = 'fc-sub-'.$aiIdx.'-'.$fi; @endphp
                            <div style="border:1px solid rgba(255,255,255,.06);border-radius:8px;margin-bottom:8px;overflow:hidden;background:rgba(255,255,255,.02);">
                                <button onclick="toggleFlashcard('{{ $cid }}')"
                                    ...
                                    <span id="{{ $cid }}-icon" style="color:var(--accent);font-size:18px;flex-shrink:0;line-height:1;transition:transform .2s;">+</span>
                                </button>
                    ...
                    <div class="sb-panel" style="background:linear-gradient(160deg,rgba(34,197,94,.07) 0%,transparent 70%);">
                        <div style="padding:16px 18px 16px;">
                            ...
                            <audio controls style="width:100%;height:36px;border-radius:50px;outline:none;accent-color:var(--accent);">
